...
class WhatsAppController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        ...
        // 2. Find user
        // Priority: Search by clean phone (standard) or by part of the name if ID is weird
    ...
    private function handleInternalActions($phone, $message, $user = null)
    {
        ...
        if (strpos($msg, 'ABSEN') !== false) {
            ...
            return $this->wa->sendMessage($phone, $this->bot->medanize("Okelah! Kirimkan lah lokasi (GPS) kau sekarang lewat WA biar kucatat absennya!"));
        }
    ...
    private function handleAttendanceLocation($phone, $request)
    {
        ...
        if (!$targetSppg) {
            return $this->wa->sendMessage($phone, $this->bot->medanize("Waduh lae, gak bisa absen kau di sini. Lokasimu ($lat, $lng) kejauhan dari SPPG bah! Harus di deket kantor SPPG lah baru bisa absen!"));
        }
        ...
        return $this->wa->sendMessage($phone, $this->bot->medanize("Mantap! Sudah kucatat absenmu di $locationName ($lat, $lng). Semangat kerjanya lae!"));
    }
}
