...
>
    ...
    <div class="flex-1 overflow-y-auto py-8 space-y-2 custom-scrollbar">
        ...
        <div class="px-8 mt-4 mb-2">
            <p class="text-[9px] font-black text-gold uppercase tracking-[0.3em]">Menu Pemasok</p>
        </div>
        <x-nav-link :href="route('offers.form')" :active="request()->routeIs('offers.form')" class="py-3">
            <svg class="w-5 h-5 mr-4" :class="active ? 'text-gold' : 'text-gray-400 group-hover:text-gold'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
            </svg>
            <span class="font-bold tracking-tight">{{ __('Ajukan Penawaran') }}</span>
        </x-nav-link>
        @endif
        ...
        @if(auth()->user()->can('access-preparation') || auth()->user()->can('access-processing') || auth()->user()->can('access-portioning') || auth()->user()->can('manage-menus'))
        <div class="px-8 mt-4 mb-2">
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-[0.3em]">Production Monitoring</p>
        </div>
        @endif

        @can('access-preparation')
        ...
        @can('manage-menus')
        <x-nav-link :href="route('production.daily-lpj.index')" :active="request()->routeIs('production.daily-lpj.*')" class="py-3">
            <svg class="w-5 h-5 mr-4" :class="active ? 'text-gold' : 'text-gray-400 group-hover:text-gold'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <span class="font-bold tracking-tight">{{ __('4. LPJ Harian') }}</span>
        </x-nav-link>
        @endcan
        ...
        <x-nav-link :href="route('reports.lpj-sppg.index')" :active="request()->routeIs('reports.lpj-sppg.*')" class="py-3">
            <svg class="w-5 h-5 mr-4" :class="active ? 'text-gold' : 'text-gray-400 group-hover:text-gold'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <span class="font-bold tracking-tight">{{ __('BERKAS LPJ SPPG') }}</span>
        </x-nav-link>
        ...
        <x-nav-link :href="route('reports.daily')" :active="request()->routeIs('reports.daily')" class="py-3">
            ...
            <span class="font-bold tracking-tight">{{ __('Upload Dokumen Harian') }}</span>
        </x-nav-link>
